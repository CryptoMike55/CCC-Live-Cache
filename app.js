...
const TARGET_MOON = 1_000_000; // 1M target
...
async function fetchBurnCache() {
  try {
    ...
    // Convert to plain number of MOON for progress (no decimals needed here)
    const decimals = 18n;
    const divisor = 10n ** decimals;
    const moonWhole = Number(valueWei / divisor); // safe at these sizes

    // 2) progress bar and percent
